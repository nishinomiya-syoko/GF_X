...
namespace EmpireClash
{
    ...
    public class BuildingManager : MonoBehaviour
    {
        ...
        {
            if (!GameManager.Instance?.ResourceManager?.CanAfford(buildingData.buildCosts) == true)
            {
        ...
        private void UpdatePlacementGhost()
        {
            ...
            {
                Vector3 placementPosition = GameManager.Instance?.MapManager?.GetNearestGridPosition(hit.point) ?? hit.point;
                currentGhost.transform.position = placementPosition;

                // 检查是否可以放置
                bool canPlace = GameManager.Instance?.MapManager?.CanPlaceBuilding(selectedBuildingData,placementPosition) == true;
        ...
        private void TryPlaceBuilding()
        {
            ...
            if (Physics.Raycast(ray, out hit, Mathf.Infinity, groundLayer))
            {
                ...
                if (GameManager.Instance?.MapManager?.CanPlaceBuilding(selectedBuildingData,placementPosition) == true)
                {
        ...
        {
            if (!GameManager.Instance?.ResourceManager?.SpendResources(buildingData.buildCosts) == true)
            {
            ...
            GameObject buildingObj = GF.Entity.GetEntity("1").gameObject;
            ...
            building.currentState = BuildingState.Building;

            // 注册到地图系统
            GameManager.Instance?.MapManager?.PlaceBuilding(buildingData, position);
        ...
        public void UpgradeBuilding(Building building)
        {
            ...
            if (GameManager.Instance?.ResourceManager?.SpendResources(upgradeCosts) == true)
            {
        ...
        //     // 从地图系统中移除
        //     GameManager.Instance?.MapManager?.RemoveBuilding(building.data,building);
