...
namespace EmpireClash
{
    ...
    public class ResourceProducer : MonoBehaviour, IResourceProducer
    {
        ...
        void Start()
        {
            GameManager.Instance?.ResourceManager?.RegisterResourceProducer(this);
        }

        void OnDestroy()
        {
            GameManager.Instance?.ResourceManager?.UnregisterResourceProducer(this);
        }
        ...
        private void ProduceResource()
        {
            ...
            // 触发生产事件
            // GameManager.Instance?.ResourceManager?.OnResourceProduced?.Invoke(productionType, productionRate);
            GameManager.Instance?.ResourceManager?.ProduceResource(productionType, productionRate);
        }
